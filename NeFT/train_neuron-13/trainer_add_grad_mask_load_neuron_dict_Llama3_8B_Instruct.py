            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        # 마스킹에서 제외할 layer 4개
                        excluded_layers = {31, 32, 34, 35}
                        
                        for k, v in model.named_parameters():
                            if v.grad is None:
                                continue
                        
                            # parameter 이름에서 layer 번호 추출
                            current_layer = None
                            ...
                            # 지정한 layer는 gradient masking을 수행하지 않음
                            if current_layer in excluded_layers:
                                continue
                        
                            if "up_proj" in k and "mlp" in k:
                                if current_layer is not None:
                                    layer_name = f"{current_layer}_in"
                        
                                    if layer_name in mask_dict:
                                        mask = mask_dict[layer_name].to(
                                            device=v.grad.device,
                                            dtype=v.grad.dtype,
                                        )
                                        mask = mask.reshape(12288, 4096)
                        
                                        v.grad.mul_(mask)
                        
                            elif "down_proj" in k and "mlp" in k:
                                if current_layer is not None:
                                    layer_name = f"{current_layer}_out"
                                    ...
                                    if layer_name in mask_dict:
                                        ...
                                        mask = mask.reshape(4096, 12288)
                        
                                        v.grad.mul_(mask)
                        
                            else:
                                # 예외 layer가 아닌 up_proj/down_proj 외 파라미터는 gradient를 0으로 설정
                                v.grad.zero_()
