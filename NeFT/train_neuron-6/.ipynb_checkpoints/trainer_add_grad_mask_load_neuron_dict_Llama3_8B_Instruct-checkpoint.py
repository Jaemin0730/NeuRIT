...
mask_dict = torch.load('./sft_neuron_mask.pt')
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        target_layers = [30, 31, 32, 36, 47]

                        for k, v in model.named_parameters():
                            if v.grad is None:
                                continue
                        
                            is_up_proj = 'up_proj' in k and 'mlp' in k
                            is_down_proj = 'down_proj' in k and 'mlp' in k
                        
                            # 1. up_proj 또는 down_proj인 경우
                            if is_up_proj or is_down_proj:
                                
                                # 현재 파라미터가 몇 번 레이어인지 확인
                                current_layer = -1
                                for i in range(48):
                                    if f'.{i}.' in k:
                                        current_layer = i
                                ...
                                # 1-1. 내가 선택한 레이어 (14, 19, 21) -> Full FT
                                if current_layer in target_layers:
                                    # v.grad를 건드리지 않고 그대로 둡니다 (100% 학습)
                                    continue 
                                    
                                # 1-2. 선택하지 않은 나머지 레이어 -> 마스킹 적용
                                else:
                                    if is_up_proj:
                                        layer_name = str(current_layer) + '_in'
                                        if layer_name in mask_dict.keys():
                                            mask = mask_dict[layer_name].to('cuda')
                                            mask = mask.reshape(13824, 5120)
                                            v.grad *= mask
                                            
                                    elif is_down_proj:
                                        layer_name = str(current_layer) + '_out'
                                        if layer_name in mask_dict.keys():
                                            mask = mask_dict[layer_name].to('cuda')
                                            mask = mask.reshape(5120, 13824)
                                            v.grad *= mask
                        
                            # 2. up/down_proj가 아닌 나머지 모든 파라미터 -> 기존 코드대로 학습 동결 (Gradient 0)
                            else:
